...
while [[ $# -gt 0 ]]; do
    case $1 in
        -h|--help)
            ...
            echo "Example: $0 1.5.0 --dry-run"
            echo "Example: $0 1.5.0 --breaking 123,125"
            exit 0
            ;;
        --dry-run)
            DRY_RUN=true
...
if ! [[ "$VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    ...
    echo "Version must follow semver format: MAJOR.MINOR.PATCH (e.g., 1.5.0)"
    exit 1
fi
...
if [ "$EDIT_CHANGELOG" = true ]; then
    ...
    # Re-extract release notes from the edited changelog
