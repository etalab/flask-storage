...
from flask_storage import DEFAULTS, NONE, Storage
...
def test_custom_f_url(app):
    files = Storage("files")
    app.configure(
        files, FS_URL="http://somewhere.net/test/", FILES_FS_URL="http://somewhere-else.net/test/"
    )
...
def test_backend_level_configuration(app):
    files = Storage("files")
    app.configure(
        files, FS_URL="http://somewhere.net/test/", FS_LOCAL_URL="http://somewhere-else.net/local/"
    )
    assert isinstance(files.backend, LocalBackend)
